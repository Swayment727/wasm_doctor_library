...
report(struct error_reporter *reporter)
{
        ...
        for (uint32_t i = 0; i < reporter->undefined_memory_use_errors_size;
             ++i) {
                printf("Undefined value of size %u bytes read from address "
                       ...
                       reporter->undefined_memory_use_errors[i]
                               .location.function_name);
        }

        for (uint32_t i = 0; i < reporter->undefined_local_use_errors_size;
             ++i) {
                printf("Undefined value of size %u bytes read from local %u. "
                       "(%s)\n",
                       reporter->undefined_local_use_errors[i].size,
                       reporter->undefined_local_use_errors[i].idx,
                       reporter->undefined_local_use_errors[i]
                               .location.function_name);
        }
        ...
        for (uint32_t i = 0; i < reporter->use_after_free_errors_size; ++i) {
                printf("Use after free of size %u bytes detected at address "
                       "%u. (%s)\n",
                       reporter->use_after_free_errors[i].size,
                       reporter->use_after_free_errors[i].address,
                       reporter->use_after_free_errors[i]
                               .location.function_name);
        }
        ...
        for (uint32_t i = 0; i < reporter->memory_leak_errors_size; ++i) {
                printf("Memory leak of size %u bytes detected at address "
                       "%u. (%s)\n",
                       reporter->memory_leak_errors[i].size,
                       reporter->memory_leak_errors[i].address,
...
void
add_undefined_memory_use(struct error_reporter *reporter, uint32_t address,
                         uint32_t size, char *function_name)
{
        struct undefined_memory_use *errors =
        ...
        errors[*errors_size - 1].location.function_name =
...
add_undefined_local_use(struct error_reporter *reporter, uint32_t idx,
                        uint32_t size, char *function_name)
{
        struct undefined_local_use *errors =
...
void
add_use_after_free(struct error_reporter *reporter, uint32_t address,
                   uint32_t size, char *function_name)
{
...
void
add_memory_leak(struct error_reporter *reporter, uint32_t address,
                uint32_t size, char *function_name)
{
...
void
add_doubled_free(struct error_reporter *reporter, uint32_t address,
                 char *function_name)
{
...
reporter_exit(struct error_reporter *reporter)
{
        for (uint32_t i = 0; i < reporter->undefined_memory_use_errors_size;
             ++i) {
                free(reporter->undefined_memory_use_errors[i]
                             .location.function_name);
        }

        for (uint32_t i = 0; i < reporter->undefined_local_use_errors_size;
             ++i) {
                free(reporter->undefined_local_use_errors[i]
                             .location.function_name);
        }

        for (uint32_t i = 0; i < reporter->use_after_free_errors_size; ++i) {
                free(reporter->use_after_free_errors[i]
                             .location.function_name);
        }
